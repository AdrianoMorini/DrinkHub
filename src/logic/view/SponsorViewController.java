...
public class SponsorViewController implements Initializable {
    ...
    void pricePrieview(ActionEvent event) {
    	...
    	controller.getBean().setType(choosing.getText());
    	controller.getBean().setTime(timing.getText());
    ...
    void sendRequest(ActionEvent event) throws IOException {
		...
		String pf = priceFinal.getText();
		
		SponsorController controller = SponsorController.getInstance();
		controller.getBean().setPrice(Double.valueOf(pf));
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		*/
		...
		if( controller.getBean().getTimeSponsor() != null) {
			...
			titleSponsor.setText("Your sponsor is still in activity. Will end in " + controller.getBean().getTimeSponsor());
		}
