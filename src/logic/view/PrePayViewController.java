...
public class PrePayViewController implements Initializable {
    ...
    void paying(ActionEvent event) throws InterruptedException, FileNotFoundException {
    	...
    	if(p) {
    		...
    		if(controller.saveSponsor()) {
    			messageImage.setImage(new Image(new FileInputStream("logic/image/tick.png")));
        		success.setText("Payment accepted");
        		specific.setText(s1);
    		}
    		else {
    			messageImage.setImage(new Image(new FileInputStream("logic/image/report.png")));
	...
	public void initialize(URL location, ResourceBundle resources) {
		double price = 0; 
		try {
			imgBan.setImage(new Image(new FileInputStream("logic/image/ban.JPG")));
